...
class ZO_Instagram_Widget extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        $accessToken = empty($instance['access_token']) ? '' : $instance['access_token'];
        ...
        if ($username != '') {
            $media_array = $this->scrape_instagram($username, $accessToken, $limit);
    ...
    function form( $instance ) {
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('username')); ?>"><?php _e('User Name', 'fajar'); ?>: <input class="widefat" id="<?php echo esc_attr($this->get_field_id('username')); ?>" name="<?php echo esc_attr($this->get_field_name('username')); ?>" type="text" value="<?php echo esc_attr($username); ?>" /></label></p>
        <p><label for="<?php echo esc_attr($this->get_field_id('access_token')); ?>"><?php _e('Access Token', 'fajar'); ?>: <input class="widefat" id="<?php echo esc_attr($this->get_field_id('access_token')); ?>" name="<?php echo esc_attr($this->get_field_name('access_token')); ?>" type="text" value="<?php echo esc_attr($accessToken); ?>" /></label></p>
    ...
    function scrape_instagram($username, $accessToken, $slice = 9) {
        if (false === ($instagram = get_transient('instagram-media-'. sanitize_title_with_dashes($username)))) {
            $accessToken = empty($accessToken) ? INSTAGRAM_ACCESS_TOKEN : $accessToken;
            $userId = $this->getInstaID($username);
            if (is_wp_error($userId)) {
                return $userId;
            }

            $remote = wp_remote_get('https://api.instagram.com/v1/users/'
                . trim($userId)
                . "/media/recent/?access_token="
                . $accessToken . '&count=' . $slice, true);
            ...
            if (!$insta_array) return new WP_Error('bad_json', __('Instagram has returned invalid data.', 'fajar'));
